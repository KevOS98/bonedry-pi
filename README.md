...
- The goal of this architecture is to maximize three things: agent capability, security, and simplicity. It is the closest to blank Pi configuration with simple permanence states across containers that I could design, while most importantly retaining the appeal of an agent handling all self-improvements, and that does mean all.

- The core functionality of this architecture relies only on SQLite and specific volume mounts. The dockerfile & compose.yml create and bind 7 mounts that enable the desired functionality.
    The current repo configuration includes a custom AGENTS.md that serves as an instruction template for models to use this architecture. You must manually update this to fit your personal preferences.

- Checkout the compose.yml and .gitignore for all missing directories in the repo and create them before running docker compose.
...
5. Edit .env and replace your key for injection
...
- Remember that Pi can autonomously handle building layers of increasing complexity on top of this architecture, if you choose so. These initial settings are meant to be simple enough for the widest range of models to handle and follow, assuming it isn't a complete potatoe. This does mean that the AGENTS.md may need to be tailored if you're running significantly incapable models, so provide the repo as context and consult a free frontier model like claude to tailor it accordingly.
